...
func checkKind(k reflect.Kind, tAmf byte) bool {
	if tAmf == amf_undefine || tAmf == amf_null {
		return true
	}

	switch k {
	case reflect.Bool:
		return tAmf == amf_true || tAmf == amf_false
	case reflect.Int, reflect.Int8, reflect.Int16,
		reflect.Int32, reflect.Int64:
		return tAmf == amf_integer
	case reflect.Uint, reflect.Uint8, reflect.Uint16,
		reflect.Uint32, reflect.Uint64:
		return tAmf == amf_integer
	case reflect.Float32, reflect.Float64:
		return tAmf == amf_double
	case reflect.Array, reflect.Slice:
		return tAmf == amf_array || tAmf == amf_byte_array
	case reflect.Struct:
		return tAmf == amf_array
	case reflect.String:
		return tAmf == amf_string || tAmf == amf_byte_array
	default:
		return false
	}
}
...
func encodeString(w io.Writer, value string, ref *ReferenceLog) (int, error) {
	...
	if isRef {
		//log.Println("l = l << 1", l, l<<1, uint32(len(value)), value)
		l = l << 1
	} else {
		//log.Println("l = l << 1 |1", l, (l<<1)|1, uint32(len(value)), value)
		l = (l << 1) | 1
	}
